...
for(i in 1:length(allSpecies)) {
  ...
  if(file.exists(paste0(vdir,'CLN_mask1/CLN_mask1_',spname,'.rdata'))) {
    mask1 <- readRDS(paste0(vdir,'CLN_mask1/CLN_mask1_',spname,'.rdata'))
  }
  ...
  for(a in myrange) {
    ...
    figfile <- paste0(fdir, 'Bay_lead-trail_vegclip/rasterplot/mask1/',mySpecies,'_',myScenario,'.png')
    if(file.exists(figfile)) {
      ...
      # Plot Bay Area map with mask1
      #figfile <- paste0(fdir, 'Bay_lead-trail_vegclip/rasterplot/mask1/',mySpecies,'_',myScenario,'.png')
      png(figfile, width=4500, height=1500, pointsize = 36)
      
      par(mfrow=c(1,3),mar=c(0.1,0.1,2,0.1))
      ...
      ## Second plot masking to current veg type mask 1
      if(file.exists(paste0(vdir,'CLN_mask1/CLN_mask1_',spname,'.rdata'))) {
        mask1p <- projectRaster(mask1,hst,method='ngb')
      
        plot(mask(fut-hst,mask1p,inverse=T),axes=F,zlim=c(-0.5,0.5),legend=F,
             col=colorRampPalette(c("red", "white", "blue"))(20))
        plot(mask(hst,mask1p,inverse=T),axes=F,zlim=c(0,1),legend=F,col=grey.colors(20,start=1,end=0),alpha=0.5,add=T)
        title('mask1')
      ...
      ## Third plot masking to current veg type mask 2
      mask2p <- projectRaster(mask2,hst,method='ngb')
      
      plot(mask(fut-hst,mask2p,inverse=T),axes=F,zlim=c(-0.5,0.5),legend=F,
           col=colorRampPalette(c("red", "white", "blue"))(20))
      plot(mask(hst,mask2p,inverse=T),axes=F,zlim=c(0,1),legend=F,col=grey.colors(20,start=1,end=0),alpha=0.5,add=T)
      title('mask2')
      ...
      # Legend
      if(i==1 & a==2) {
        lfile <- paste0(fdir, 'Bay_lead-trail_vegclip/rasterplot/00_Legend.png')
